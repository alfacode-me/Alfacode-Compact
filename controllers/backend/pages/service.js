...
module.exports = {
    path: '/backend/pages/service',
    router: router
};

router.get('/', (req, res) => {
    modelPages.get('service', (page) => {
        modelUsers.get(page.page.user, (user) => {
            ...
            res.render('backend/pages/service', {
                title: 'Halaman "Layanan"',
                subtitle: 'mengelola halaman "Layanan"',
                layout: 'backend',
                plugin: {
                    froalaEditor: true
                },
...
router.post('/save', (req, res) => {
    ...
    modelPages.update('service', data, (data) => {
        if (data.status) {
            res.json({
                status: 1
            });
